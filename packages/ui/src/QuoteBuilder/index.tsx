        ...
        <Box sx={{flex: 1, display: 'flex', flexDirection: 'column', paddingLeft: '6px', paddingRight: '6px'}}>
            <Box sx={{display: 'flex', marginTop: '6px', marginBottom: '6px'}}>
                ...
                <Box sx={{display: 'flex', flex: 1}}>
                    ...
                    <TextField size="small" value={props.quoteId} fullWidth label="Quote Number" {...({InputProps: {startAdornment: <InputAdornment position="start">#</InputAdornment>}} as any)} />
                </Box>
            </Box>
                ...
                <Box sx={{paddingLeft: '10%', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end'}}>
                    <Box>
                        <Box sx={{display: 'flex', alignItems: 'center',  justifyContent: 'space-between'}}>
                            <Typography sx={{fontSize: 'large'}}>Subtotal:</Typography>
                            <Typography sx={{fontSize: 'large'}}>{formatter.format(total)}</Typography>
                        </Box>
                        <Box sx={{display: 'flex', alignItems: 'center',  justifyContent: 'space-between'}}>
                            <Typography sx={{fontSize: 'large'}}>Total GST:</Typography>
                            <Typography sx={{fontSize: 'large'}}>{formatter.format((total * 1.15) - (total))}</Typography>
                        </Box>
                        <Divider />
                        <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                            <Typography sx={{fontSize: 'large'}}>Total:</Typography>
                            <Typography sx={{fontWeight: 'bold', fontSize: '25px'}}>{formatter.format(total * 1.15)}</Typography>
                        </Box>
                    </Box>
                </Box>
            </Box>
