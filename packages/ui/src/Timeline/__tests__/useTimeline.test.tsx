...
describe('useTimeline', () => {
  ...
  // ── Controlled selectedItemIds ──────────────────────────────────

  it('does not re-call onSelect when selectedItemIds prop changes to a content-identical array', () => {
    ...
    // First render fires onSelect once via the useEffect
    expect(onSelect).toHaveBeenCalledTimes(1);

    // Re-render with a NEW array containing the SAME items.
    // This simulates what happens when a parent passes inline arrays
    // like `selectedItemIds={selectedId ? [selectedId] : []}` on every render.
    // The selection hasn't actually changed, so onSelect should NOT fire again.
    rerender(
      <Probe
        props={{ ...props, selectedItemIds: ['item-1'] }}
        onResult={() => {}}
      />,
    );
