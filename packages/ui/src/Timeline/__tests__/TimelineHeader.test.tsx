...
describe('TimelineHeader', () => {
  ...
  it('drops day tier when pxPerDay < 14 (tight zoom)', () => {
    // pxPerMs * 86400000 must be < 14.
    ...
    renderHeader({
      ...
      geometry: {
        ...
        sidebarWidth: 0,
        timelineWidth: 800,
        pxPerMs: 800 / (365 * 86400000), // 800px over 365 days → ~2.19 px/day (< 14, tier dropped)
        pxPerStep: 2.19,
        stepDurationMs: 86400000,
      },
    });
    const header = document.querySelector('[data-timeline-header]')!;
    // Only month tier should render (day tier dropped when < 14px/day)
  ...
  it('shows shortened day labels when pxPerDay < 30', () => {
    // 25px/day → between 14 and 30, so shows day numbers only
    renderHeader({
      step: 'day',
      start: d('2025-06-01'),
      end: d('2025-06-08'),
    ...
    // Slightly above 30, so full labels. Let's make it narrower.
    renderHeader({
      step: 'day',
      start: d('2025-06-01'),
      end: d('2025-06-08'),
