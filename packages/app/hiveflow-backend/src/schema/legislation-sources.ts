...
export async function fetchDocument(url: string): Promise<FetchedDocument> {
  ...
  // ── Phase 1.5: NZ legislation — use HTML directly ─────────
  // legislation.govt.nz HTML has proper heading structure.
  // Prefer it over PDF extraction which produces poor text quality.
  if (source.name === 'NZ Legislation') {
    try {
      ...
      if (htmlResp.ok) {
        ...
        const md = htmlToMarkdown(html);
        if (md.length > 500) {
          // Also try to discover "as at" date for the PDF URL (for PDF viewer)
...
export async function fetchVersions(
  ...
  // NZ legislation: versions page is JS-only. Extract "as at" date
  // from the main HTML page as the single discoverable version.
