...
export function useTimeline(props: TimelineProps): UseTimelineReturn {
  ...
  const sidebarWidth = groups && groups.length > 0 ? DEFAULT_SIDEBAR_WIDTH : 0;
  ...
  useEffect(() => {
    if (props.selectedItemIds !== undefined && props.selectedItemIds !== prevControlledItemIdsRef.current) {
      prevControlledItemIdsRef.current = props.selectedItemIds;
      setSelectedItemIds(props.selectedItemIds);
    }
  }, [props.selectedItemIds]);
  useEffect(() => {
    if (props.selectedLinkIds !== undefined && props.selectedLinkIds !== prevControlledLinkIdsRef.current) {
